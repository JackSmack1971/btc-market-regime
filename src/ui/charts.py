...
def plot_score_gauge(score: float):
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge = {
            'axis': {'range': [-5, 5], 'tickwidth': 1, 'tickcolor': "#c9d1d9"},
            'bar': {'color': "#39FF14" if score > 1.5 else "#FF3131" if score < -1.5 else "#FFCC00"},
            'bgcolor': "rgba(0,0,0,0)",
            'borderwidth': 2,
            'bordercolor': "#30363d",
            'steps': [
                {'range': [-5, -3], 'color': 'rgba(255, 49, 49, 0.4)'},
                {'range': [-3, -1.5], 'color': 'rgba(255, 49, 49, 0.1)'},
                {'range': [-1.5, 1.5], 'color': 'rgba(255, 204, 0, 0.1)'},
                {'range': [1.5, 3], 'color': 'rgba(57, 255, 20, 0.1)'},
                {'range': [3, 5], 'color': 'rgba(57, 255, 20, 0.4)'}
            ],
            'threshold': {
...
def plot_confluence_heatmap(mtf_results: Dict[str, Any]):
    ...
    scores = [mtf_results[h]['score'] for h in horizons]
